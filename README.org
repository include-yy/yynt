...
If a project object with the same name already exists in =yynt-project-list=, =yynt-create-project= will replace the old object with the new one.
...
- =:collect-2=, function that returns a list of absolute paths of the
  ...
  :collect-2 (yynt-c2 "^2")
  #+end_src
- =:excluded-fn-2=, function that takes build object =bobj= and a subdirectory =sd= of
  =bobj= as parameter and returns a predicate function to determine whether the
  files and folders in =sd= should be excluded during publication
